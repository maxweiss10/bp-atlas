...
def _hours(t):
    """Hours implied by a displayed string - '2-3 d', '<1 d', '3+ wk', '15-30 min'.

    The sort value is read back off the string the column actually prints, so a
    column can never order itself differently from what the reader sees.
    """
    if t in ODD:
        return ODD[t]
    m = re.fullmatch(r'([<>~]?)([\d.]+)(?:\s*[-+]\s*([\d.]+)?)?\s*(min|h|d|wk)', t)
    if not m:
        raise ValueError(f'unparseable kinetics string: {t!r}')
    lead, lo, hi, unit = m.group(1), float(m.group(2)), m.group(3), m.group(4)
    if hi is not None:                       # '2-3 d' sorts on its midpoint
        v = (lo + float(hi)) / 2
...
CLASS_KIN = {
    'ACEi':    dict(on=_('1 h', 'C'), pk=_('2-6 h', 'C'), full=_('1-2 wk', 'C'),
                    off=_('2-3 d', 'C'), hl='effective half-life 10-13 h'),
    'ARB':     dict(on=_('2 h', 'C'), pk=_('3-6 h', 'C'), full=_('2-4 wk', 'C'),
                    off=_('2-4 d', 'C'), hl='half-life 6-15 h'),
    'BB':      dict(on=_('2-4 h', 'C'), pk=_('2-4 h', 'C'), full=_('1-2 wk', 'C'),
                    off=_('2-3 d', 'C'), hl='half-life 6-12 h', taper=True),
    'DHP':     dict(on=_('2-5 h', 'C'), pk=_('2-8 h', 'C'), full=_('1-2 wk', 'C'),
                    off=_('2-3 d', 'C'), hl='half-life 8-16 h'),
    'thiazide': dict(on=_('2 h', 'C'), pk=_('4 h', 'C'), full=_('2-4 wk', 'C'),
                     off=_('2-3 d', 'C'), hl='half-life 6-15 h'),
}
...
# ---------------------------------------------------------------- ACE inhibitors
'benazepril': dict(
    on=_('<1 h', 'L'), pk=_('2-4 h', 'L'), full=_('1-2 wk', 'C'),
    off=_('2 d', 'K'), hl='effective half-life 10-11 h; steady state after 2-3 doses',
...
'captopril': dict(
    on=_('15-30 min', 'K'), pk=_('1-1.5 h', 'L'), full=_('3+ wk', 'L'),
    off=_('1 d', 'K'), hl='half-life under 2 h - the shortest-acting agent here', reb='ok',
...
'enalapril': dict(
    on=_('1 h', 'L'), pk=_('4-6 h', 'L'), full=_('1-2 wk', 'C'),
    off=_('2-3 d', 'K'), hl='effective half-life 11-14 h', reb='ok',
    q=['Label: onset of antihypertensive activity was seen at one hour with peak reduction of blood pressure achieved by four to six hours.',
       ...
       'Label: abrupt withdrawal has not been associated with a rapid increase in blood pressure.']),
'fosinopril': dict(
    on=_('<1 h', 'L'), pk=_('2-6 h', 'L'), full=_('3+ wk', 'L'),
    off=_('2-3 d', 'K'), hl='half-life ~12 h', reb='ok',
...
'lisinopril': dict(
    on=_('1 h', 'L'), pk=_('6 h', 'L'), full=_('1-2 wk', 'C'),
    off=_('2-3 d', 'K'), hl='effective half-life 12 h', reb='ok',
    q=['Label: onset of antihypertensive activity was seen at one hour, with peak reduction of blood pressure achieved by 6 hours.',
       ...
       'Label: abrupt withdrawal has not been associated with a rapid increase in blood pressure.']),
'moexipril': dict(
    on=_('1 h', 'L'), pk=_('3-6 h', 'L'), full=_('4 wk', 'L'),
    off=_('1-2 d', 'K'), hl='half-life 2-9 h',
...
'perindopril': dict(
    on=_('1 h', 'C'), pk=_('4 h', 'L'), full=_('3+ wk', 'L'),
    off=_('3-6 d', 'K'), hl='perindoprilat accumulates to steady state over 3-6 days; terminal half-life 30-120 h from slow ACE dissociation',
...
'quinapril': dict(
    on=_('<1 h', 'L'), pk=_('2-4 h', 'L'), full=_('1-2 wk', 'L'),
    off=_('1-2 d', 'K'), hl='effective accumulation half-life ~3 h, terminal 25 h',
...
'ramipril': dict(
    on=_('1-2 h', 'L'), pk=_('3-6 h', 'L'), full=_('2-4 wk', 'L'),
    off=_('3 d', 'K'), hl='half-life 13-17 h in the therapeutic range', reb='ok',
    q=['Label: lowered blood pressure within 1 to 2 hours, with peak reductions achieved 3 to 6 hours after dosing.',
       ...
       'Label: abrupt withdrawal has not resulted in a rapid increase in blood pressure.']),
'trandolapril': dict(
    on=_('1 h', 'C'), pk=_('4-10 h', 'L'), full=_('1 wk', 'L'),
    off=_('4-8 d', 'K'), hl='effective half-life 22.5 h', reb='ok',
...
# ---------------------------------------------------------------- ARBs
'azilsartan': dict(
    on=_('2 h', 'C'), pk=_('3 h', 'K'), full=_('2 wk', 'L'),
    off=_('2 d', 'K'), hl='half-life ~11 h; steady state within 5 days', reb='ok',
...
'candesartan': dict(
    on=_('2 h', 'C'), pk=_('3-4 h', 'K'), full=_('4-6 wk', 'L'),
    off=_('2 d', 'K'), hl='half-life ~9 h', reb='ok',
...
'irbesartan': dict(
    on=_('1st dose', 'L'), pk=_('3-6 h', 'L'), full=_('2 wk', 'L'),
    off=_('>7 d', 'L'), hl='half-life 11-15 h; steady state within 3 days', reb='ok',
...
'losartan': dict(
    on=_('2 h', 'C'), pk=_('6 h', 'L'), full=_('3-6 wk', 'L'),
    off=_('1-2 d', 'K'), hl='losartan half-life ~2 h, active metabolite 6-9 h', reb='ok',
...
'olmesartan': dict(
    on=_('2 h', 'C'), pk=_('2 h', 'K'), full=_('2-4 wk', 'L'),
    off=_('2-3 d', 'K'), hl='half-life ~13 h; steady state within 3-5 days', reb='ok',
...
'telmisartan': dict(
    on=_('1st dose', 'L'), pk=_('3 h', 'K'), full=_('4 wk', 'L'),
    off=_('3-7 d', 'L'), hl='half-life ~24 h', reb='ok',
...
'valsartan': dict(
    on=_('2 h', 'L'), pk=_('6 h', 'L'), full=_('4 wk', 'L'),
    off=_('1-2 d', 'K'), hl='half-life ~6 h', reb='ok',
...
# ---------------------------------------------------------------- beta blockers
'atenolol': dict(
    on=_('2-4 h', 'K'), pk=_('2-4 h', 'K'), full=_('1-2 wk', 'L'),
    off=_('1-2 d', 'K'), hl='half-life 6-7 h, much longer in the elderly and in renal failure', taper=True,
...
'betaxolol': dict(
    on=_('3-4 h', 'K'), pk=_('3-4 h', 'L'), full=_('1-2 wk', 'L'),
    off=_('3-4 d', 'K'), hl='half-life 14-22 h; steady state in 5-7 days', taper=True,
...
'bisoprolol': dict(
    on=_('2-4 h', 'K'), pk=_('2-4 h', 'K'), full=_('1 wk', 'L'),
    off=_('2-3 d', 'K'), hl='half-life 9-12 h; steady state within 5 days', taper=True,
...
'carvedilol': dict(
    on=_('1-2 h', 'L'), pk=_('1-2 h', 'L'), full=_('1-2 wk', 'L'),
    off=_('2 d', 'K'), hl='half-life 7-10 h', taper=True,
...
'metoprolol': dict(
    on=_('6-7 h', 'K'), pk=_('7 h', 'K'), full=_('1 wk', 'L'),
    off=_('1 d', 'K'), hl='plasma half-life 3-7 h, extended by the succinate ER matrix', taper=True,
...
'nebivolol': dict(
    on=_('2-4 h', 'K'), pk=_('2-4 h', 'K'), full=_('2 wk', 'L'),
    off=_('2-4 d', 'K'), hl='effective half-life ~12 h (19 h in CYP2D6 poor metabolizers)', taper=True,
...
'propranolol': dict(
    on=_('6 h', 'K'), pk=_('6 h', 'K'), full=_('1-3 wk', 'L'),
    off=_('2 d', 'K'), hl='half-life ~10 h for the long-acting capsule', taper=True,
...
'amlodipine': dict(
    on=_('2-6 h', 'K'), pk=_('6-12 h', 'K'), full=_('1-2 wk', 'L'),
    off=_('7 d', 'K'), hl='half-life 30-50 h; steady state only after 7-8 days - the slowest on and off',
...
'diltiazem': dict(
    on=_('2-3 h', 'K'), pk=_('3-6 h', 'L'), full=_('2 wk', 'L'),
    off=_('1-2 d', 'K'), hl='steady-state half-life 5-10 h for the once-daily ER capsule',
...
'felodipine': dict(
    on=_('2-5 h', 'L'), pk=_('2-5 h', 'L'), full=_('2 wk', 'L'),
    off=_('2-3 d', 'K'), hl='half-life 11-16 h for the ER tablet',
...
'isradipine': dict(
    on=_('2-3 h', 'L'), pk=_('2-3 h', 'L'), full=_('2-4 wk', 'L'),
    off=_('1-2 d', 'K'), hl='early half-life 1.5-2 h, terminal ~8 h',
...
'nicardipine': dict(
    on=_('30 min', 'K'), pk=_('1-2 h', 'L'), full=_('3 d', 'K'),
    off=_('1 d', 'K'), hl='fast early half-life 2-4 h, terminal 8.6 h',
...
'nifedipine': dict(
    on=_('2-6 h', 'K'), pk=_('6 h', 'K'), full=_('1-2 wk', 'L'),
    off=_('1-2 d', 'K'), hl='half-life ~7 h as the ER tablet (2 h for the immediate-release capsule)', reb='ok',
...
'nisoldipine': dict(
    on=_('4-6 h', 'K'), pk=_('6-12 h', 'K'), full=_('1-2 wk', 'C'),
    off=_('2-3 d', 'K'), hl='half-life ~14 h',
...
'verapamil': dict(
    on=_('4-7 h', 'K'), pk=_('7-9 h', 'K'), full=_('1-2 wk', 'L'),
    off=_('2-3 d', 'K'), hl='half-life ~12 h for the ER capsule, and it lengthens during chronic dosing',
...
'hydrochlorothiazide': dict(
    on=_('2 h', 'L'), pk=_('4 h', 'L'), full=_('2-4 wk', 'L'),
    off=_('2-3 d', 'K'), hl='half-life 6-15 h; a single dose diureses for 6-12 h',
...
'chlorthalidone': dict(
    on=_('2-3 h', 'K'), pk=_('4-6 h', 'K'), full=_('2-4 wk', 'L'),
    off=_('7-14 d', 'K'), hl='half-life 40-60 h - by far the longest of the thiazide-type agents',
...
'indapamide': dict(
    on=_('1-2 h', 'K'), pk=_('2 h', 'K'), full=_('4 wk', 'L'),
    off=_('3-4 d', 'K'), hl='half-life ~14 h in whole blood, 26 h terminal',
...
'furosemide': dict(
    on=_('<1 h', 'L'), pk=_('1-2 h', 'K'), full=_('3 d', 'K'),
    off=_('<1 d', 'K'), hl='half-life ~2 h; a single oral dose diureses for 6-8 h',
...
'spironolactone': dict(
    on=_('1-2 d', 'K'), pk=_('1-2 d', 'K'), full=_('2 wk', 'L'),
    off=_('2-3 d', 'K'), hl='spironolactone half-life 1-2 h, but the active metabolites run 10-35 h',
...
'eplerenone': dict(
    on=_('1-2 d', 'K'), pk=_('1-2 d', 'K'), full=_('4 wk', 'L'),
    off=_('7 d', 'L'), hl='half-life 3-6 h; steady state within 2 days', reb='ok',
...
'amiloride': dict(
    on=_('2 h', 'K'), pk=_('6-10 h', 'L'), full=_('2 wk', 'C'),
    off=_('1-2 d', 'K'), hl='half-life 6-9 h; the effect on electrolyte excretion lasts about 24 h',
